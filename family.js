...
var defaultRootName = 'Lolo Magdaleno' // Someone in your family
...
function getEntries(text) {
  ...
  while (i < lines.length) {
    ...
    if (toks.includes('')) throw 'Misformatted line ' + i + ': ' + key
...
function collides(left, right, divs) {
  var layers = {}
  for (var [name, pt] of Object.entries(left).concat(Object.entries(right))) {
...
function getVisibleNodes(
  ...
  if (isPerson(name)) {
    ...
    let children =
      !path.desc && path.downsLeft === 0 ? [] : getChildren(name, neighbours)
    let result = new Set([name])
      .union(getNodes(leftParent, {}))
      .union(getNodes(rightParent, {}))
    for (let child of children) {
      result = result.union(
        getNodes(child, { allowUp: false, downsLeft: path.downsLeft - 1 })
      )
    }
    return result
  }
...
function dumbLayout(name, pred, neighbours, divs, visibleNodes) {
  ...
  if (isPerson(name)) {
    ...
    let [leftParent, rightParent] = name.split(' + ')
    let children = getChildren(name, neighbours).filter((x) =>
      visibleNodes.has(x)
    )
...
function boundingBox(layout, divs) {
  function xbound(entry, sign) {
    ...
    return (
      pt.x +
      (isUnion(name) ? 0 : sign * (paddingAmount + divs[name].offsetWidth / 2))
    )
  }
  return {
    bottomLeft: {
...
function adjustUnions(neighbours, layout, divs) {
  for (var node of Object.keys(layout)) {
    ...
    var parentBottom = Math.max(
      layout[p1].y + divs[p1].offsetHeight / 2,
    ...
    for (var child of children) {
      childTop = Math.min(
        childTop,
        layout[child].y - divs[child].offsetHeight / 2
      )
    }
    if (childTop < parentBottom) {
      errorOut(
        'Union ' + node + ' overlapped above/below. Try increasing lineHeight'
      )
    }
  ...
  if (entries[name]) {
    for (var data of entries[name]) {
      if (data.startsWith('l: ')) {
        ...
        if (birth !== '') {
          lifespanDiv.appendChild(
            document.createTextNode(birth + (death === '' ? '\u2013' : ''))
          )
        }
        if (birth !== '' && death !== '') {
      ...
      drawLine(
        {
          x:
            layout[person].x +
            ((isLeftPersonOfUnion ? 1 : -1) * divs[person].offsetWidth) / 2,
          y: layout[person].y,
        },
        { x: layout[union].x, y: layout[union].y },
        lineClass
...
function scrollToElement(element) {
  ...
  const elementMiddleY =
    window.pageYOffset + elementRect.top + element.offsetHeight / 2
  const y = elementMiddleY - window.innerHeight / 2
  const elementMiddleX =
    window.pageXOffset + elementRect.left + element.offsetWidth / 2
...
function traverse(
  ...
  if (
    mode == 'drawConnections' &&
    layout.hasOwnProperty(name) &&
    layout.hasOwnProperty(pred)
  ) {
    if (
      isUnion(name) &&
      name.split(' + ').includes(pred) &&
...
function redraw() {
  ...
  for (var kind of ['root', 'ancestor', 'blood', 'descendant', 'other']) {
    for (var el of Array.from(document.getElementsByClassName('pos-' + kind))) {
